...
class Account extends Component {
  constructor(props) {
    ...
    props.web3.eth.getBalance(props.web3Accounts[0], function(err, result) {
      document.getElementById("EtherBalance").innerHTML = props.web3.fromWei(
        result,
        "ether"
      );
    });
  ...
  componentDidMount() {
    this.props.passageInstance
      .isUserGod({ from: this.props.web3Accounts[0] })
      .then(result => {
  ...
  render() {
    return (
      <div>
        <AnnotatedSection
          ...
          panelContent={
            <div>
              ...
              <div>
                ETH balance : <span id="EtherBalance" />
              </div>
            </div>
          }
        />
                      ...
                      (certification, index) => (
                        <tr key={index}>
                          <td style={{ width: "45px" }}>
                            ...
                            certification.imageUrl.length > 0 ? (
                              <img
        ...
        />
        {this.state.isGod && <UpdateGodUser />}
      </div>
    );
  }
}
